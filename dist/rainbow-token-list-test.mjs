import { z } from 'zod';
import { pick, isPlainObject, mapValues, isString, isEmpty, partition, filter, matchesProperty, map, toLower, mapKeys, unionBy, compact, merge, uniq, keyBy, some, find } from 'lodash-es';
...
const TokenListItemSchema = z
    ...
    .url()
    .nonempty();
const TokenListTypeSchema = z.record(TokenListItemSchema);
const TOKEN_LISTS = {
...
const TokenListEnumSchema = z.enum([
    'aave',
    'coingecko',
    'dharma',
    'kleros',
...
const TokenDeprecationSchema = z.object({
 ...
 */
const RawContractMapTokenSchema = z.object({
    address: z.string(),
 ...
 * Raw token data that is loaded from the JSON files.
 */
const RawEthereumListsTokenSchema = z.object({
...
const deeplyTrimAllTokenStrings = (token) => {
    return mapValuesDeep(token, (v) => (isString(v) ? v.trim() : v));
};
...
async function mapDir({ dir, fileMap, limit = 10 }) {
    ...
    const resultPromises = files.map(file => async () => await fileMap(resolve(dir, file)));
...
const loadTokenOverrides = async () => {
    const overrides = loadFromEndpoint(REMOTE_TOKEN_OVERRIDES_ENDPOINT, OFFLINE_TOKEN_OVERRIDES);
    return overrides;
};
/**
...
const TokenListStoreRecord = z.record(TokenListStore);
...
async function build() {
    ...
    const defaultSources = merge({}, ...sources.default);
    const allKnownTokenAddresses = uniq(compact([
    ...
    function resolveTokenInfo(tokenAddress) {
        ...
        const lists = pick(tokenListTokens, Object.keys(tokenListTokens).filter((list) => some(tokenListTokens[list].tokens, matchToken)));
    ...
    console.log('# of "isRainbowCurated" tokens: ', filter(tokens, matchesProperty('extensions.isRainbowCurated', true)).length);
    console.log('# of "isVerified" tokens: ', filter(tokens, matchesProperty('extensions.isVerified', true)).length);
    return tokens;
}
